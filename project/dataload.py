...
DEFAULT_FOLDER_PATH = "C:\\Users\\User\\OneDrive - Universidad EAFIT\\Escritorio\\Project1OS-main\\Project1OS-main\\datasets"
# Set the number of cores you want to use
NUM_CORES = 1  # Change this value to use a different number of cores
...
def read_files_in_multi_core(file_paths, num_cores):
    ...
    if num_cores > available_cores:
        print(f"[bold red]Error:[/bold red] El número de cores especificado ({num_cores}) es mayor que el número de cores disponibles ({available_cores}).")
        sys.exit(1)
...
def main():
    ...
    parser.add_argument("-f", "--folder", help="Carpeta con archivos CSV")
    ...
    folder = args.folder if args.folder else DEFAULT_FOLDER_PATH
...
# Ejecutar el script
# python dataload.py [OPCIONES -s, -m or nothing]
